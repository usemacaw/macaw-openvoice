...
- `tag_audio_events` parameter on `POST /v1/audio/transcriptions` (default: `true`) — enables annotation of non-speech audio events in STT output; engine-dependent feature (#sprint-U)
- `audio_event` word type on `WordResponse` model — `word_type` field distinguishes regular words from audio events (laughter, music, applause) (#sprint-U)
- `use_multi_channel` parameter on `POST /v1/audio/transcriptions` (default: `false`) — enables per-channel transcription of multi-channel audio (#sprint-U)
- `channel_index` field on `WordResponse` model — identifies which audio channel produced each word in multi-channel transcription (#sprint-U)
- PVC (Professional Voice Cloning) endpoints: `POST /v1/voices/pvc` (create project), `POST /v1/voices/pvc/{id}/samples` (upload samples with format/size/count validation), `POST /v1/voices/pvc/{id}/train` (start training, 202 Accepted), `GET /v1/voices/pvc/{id}` (status), `DELETE /v1/voices/pvc/{id}` (delete) — all return 503 when no training backend configured (#sprint-T)
- `VoiceTrainingBackend` ABC in `macaw/workers/tts/training/interface.py` — `create_project()`, `add_samples()`, `train()`, `get_training_status()`, `get_trained_voice_id()`, `post_load_hook()` for external voice training engine integration (#sprint-T)
- PVC security constraints: max 20 MB per sample, max 25 samples per project, audio content type validation against `ALLOWED_AUDIO_CONTENT_TYPES` (#sprint-T)
- ADR-014: Professional Voice Cloning (PVC) Fine-Tuning architecture decision record — async training, external-only pattern, security constraints (#sprint-T)
- Voice changer endpoint `POST /v1/speech-to-speech/{voice_id}` — accepts multipart audio upload, validates content type and file size (50 MB max), returns 503 when no VC engine configured (#sprint-Q)
- `VoiceChangerBackend` ABC in `macaw/workers/voice_changer/interface.py` — `load()`, `convert()`, `capabilities()`, `post_load_hook()` methods for external voice conversion engine integration (#sprint-Q)
- `VoiceChangerCapabilities` frozen dataclass: `supports_streaming`, `supports_voice_reference`, `max_audio_duration_s`, `supported_sample_rates` (#sprint-Q)
- `vc_worker.proto` gRPC service definition — `VoiceChange` server-streaming RPC with `VoiceChangeRequest`/`VoiceChangeChunk` messages (#sprint-Q)
- ADR-011: Voice Changer architecture decision record — external-only engine pattern, no built-in VC implementation (#sprint-Q)
- Sound effects generation endpoint `POST /v1/sound-generation` — accepts text prompt, duration (0.5-30s), prompt_influence (0-1), loop flag, returns generated audio; 503 when no SFX engine configured (#sprint-R)
- `SoundEffectBackend` ABC in `macaw/workers/sound_effects/interface.py` — `load()`, `generate()`, `capabilities()`, `post_load_hook()` methods for external sound effect engine integration (#sprint-R)
- `SoundEffectCapabilities` frozen dataclass: `max_duration_s`, `min_duration_s`, `supports_loop`, `supports_prompt_influence` (#sprint-R)
- ADR-012: Sound Effects Generation architecture decision record — external-only engine pattern (#sprint-R)
- Dubbing endpoint `POST /v1/dubbing` — accepts source audio + target language, returns async job ID (202 Accepted); status via `GET /v1/dubbing/{id}`, audio download via `GET /v1/dubbing/{id}/audio/{lang}`, delete via `DELETE /v1/dubbing/{id}` (#sprint-S)
- `TranslationBackend` ABC in `macaw/server/dubbing/orchestrator.py` — pluggable text translation interface for dubbing pipeline (#sprint-S)
- `DubbingOrchestrator` coordinator for STT -> translate -> TTS dubbing pipeline (#sprint-S)
- `DubbingNotFoundError` typed exception with 404 error handler (#sprint-S)
- ADR-013: Dubbing Pipeline architecture decision record — async job pattern, pluggable translation (#sprint-S)
- Multi-speaker dialogue endpoint `POST /v1/text-to-dialogue` — accepts array of `{text, voice_id}` segments, synthesizes each sequentially with the correct voice, and streams concatenated audio (#sprint-P)
- Per-segment voice resolution for dialogue: each `voice_id` independently resolves saved voices (`voice_` prefix), preset voices, or default voice (#sprint-P)
- Max 10 unique voices per dialogue request enforced (422 if exceeded), max 100 segments per request (#sprint-P)
- Streaming dialogue response: WAV header emitted immediately, PCM chunks from each segment streamed as they arrive from the TTS worker (#sprint-P)
- ADR-010: Multi-Speaker TTS (Dialogue) architecture decision record (#sprint-P)
- SSML support for TTS: `enable_ssml_parsing` field on `SpeechRequest` (REST) and `TTSSpeakCommand` (WebSocket) — parses `<break>`, `<prosody>`, `<emphasis>`, `<say-as>`, `<phoneme>` tags into engine-neutral directives (#sprint-O)
- `SSMLParser` module (`macaw/postprocessing/ssml/`) with fail-open parsing via stdlib `xml.etree.ElementTree` — unsupported tags stripped, inner text preserved (#sprint-O)
- SSML directive-to-engine mapping: `<break>` generates silence PCM, `<prosody rate>` maps to speed multiplier, `<emphasis>` adjusts speech rate (#sprint-O)
- `supports_ssml` capability flag on `TTSEngineCapabilities` for runtime feature detection (#sprint-O)
- `apply_ssml_directives()` non-abstract method on `TTSBackend` ABC for engine-specific SSML handling (#sprint-O)
- ADR-009: SSML Support — documents supported subset, parsing strategy, and engine mapping approach (#sprint-O)
- Multi-context TTS WebSocket endpoint `GET /v1/text-to-speech/{voice_id}/multi-stream-input` — supports multiple independent TTS contexts over a single WebSocket connection with `context_id` tagging, matching ElevenLabs multi-stream-input API (#sprint-N)
- `WSContextManager` for tracking per-connection TTS context lifecycle (create, active, flushing, closed) with max context limits and inactivity timeout (#sprint-N)
- Multi-context protocol messages: `initialize_connection`, `send_text`, `flush_context`, `close_context`, `close_socket`, `keep_alive` with typed Pydantic models (#sprint-N)
- Concurrent TTS synthesis with per-connection semaphore limiting GPU memory usage, audio chunks interleaved with `context_id` field (#sprint-N)
- `MultiContextSettings` configuration group: `MACAW_MULTI_CONTEXT_MAX_CONTEXTS` (default 10), `MACAW_MULTI_CONTEXT_INACTIVITY_TIMEOUT_S` (default 20s), `MACAW_MULTI_CONTEXT_MAX_CONCURRENT_TTS` (default 4) env vars (#sprint-N)
- ADR-008: Multi-Context WebSocket for TTS architecture decision record (#sprint-N)
- `cloud_storage_url` parameter on `POST /v1/audio/transcriptions` — download audio from HTTPS URL (S3 pre-signed, GCS, any HTTPS) as alternative to file upload, with SSRF prevention and configurable size/timeout limits (#sprint-M)
- `AudioDownloadError` exception with HTTP 502 error handler for cloud storage download failures (#sprint-M)
- `STTDownloadSettings` configuration group: `MACAW_STT_MAX_DOWNLOAD_SIZE_BYTES` (default 2GB), `MACAW_STT_DOWNLOAD_TIMEOUT_S` (default 120s) env vars (#sprint-M)
- `commit_strategy` parameter on `session.configure` WebSocket command — controls whether VAD auto-commits segments ("vad", default) or client sends explicit `commit` commands ("manual") (#sprint-L)
- `commit` short alias for `input_audio_buffer.commit` WebSocket command — accepted alongside the existing long form (#sprint-L)
- `previous_text`, `next_text`, `previous_request_ids`, `next_request_ids` fields on `SpeechRequest` (REST) and `TTSSpeakCommand` (WebSocket) for TTS cross-generation prosody continuity (#sprint-L)
- Proto fields 19-22 on `SynthesizeRequest` for cross-generation continuity transport — `previous_text`, `next_text`, `previous_request_ids`, `next_request_ids` (#sprint-L)
- Transcript export formatters module (`macaw/postprocessing/formatters/`) with SRT, TXT, and HTML formatters — each supports include_speakers, include_timestamps, and max_chars_per_segment options (#sprint-K)
- `additional_formats` parameter on `POST /v1/audio/transcriptions` — accepts JSON array of export format requests, returns base64-encoded content in `verbose_json` response (#sprint-K)
- `AdditionalFormatResponse` Pydantic model and `additional_formats` field on `VerboseTranscriptionResponse` (#sprint-K)
- `output_format` parameter on `SpeechRequest` (REST) and `TTSSpeakCommand` (WebSocket) for ElevenLabs-style compound format strings like `mp3_44100_128`, `pcm_16000`, `opus_48000_64` — takes precedence over `response_format`/`codec` when provided (#sprint-J)
- `OutputFormat` parser module (`macaw/codec/output_format.py`) with validation of codec, sample rate, and bitrate against per-codec allowed values (#sprint-J)
- PCM resampling in TTS output path: runtime resamples from native TTS sample rate to target rate before codec encoding — supports all codecs at all allowed sample rates (#sprint-J)
- `resample_pcm_bytes()` helper in `macaw/_dsp.py` for resampling raw 16-bit PCM bytes between sample rates (#sprint-J)
- WebSocket TTS now applies codec encoding to binary audio frames (previously sent raw PCM regardless of `codec` field) (#sprint-J)
- Pronunciation dictionary module (`macaw/server/pronunciation/`) with filesystem store, Pydantic models, and text applicator for alias/phoneme rules (#sprint-H)
- `POST /v1/pronunciation-dictionaries/add-from-rules`, `GET /v1/pronunciation-dictionaries`, `GET /v1/pronunciation-dictionaries/{id}`, `DELETE /v1/pronunciation-dictionaries/{id}`, `POST /v1/pronunciation-dictionaries/{id}/rules`, `DELETE /v1/pronunciation-dictionaries/{id}/rules` CRUD endpoints (#sprint-H)
- `pronunciation_dictionary_locators` field on `SpeechRequest` (REST) and `TTSSpeakCommand` (WebSocket) for applying alias rules before TTS synthesis — max 3 dictionaries per request (#sprint-H)
- `PronunciationSettings` configuration group: `MACAW_PRONUNCIATION_STORE_PATH` env var (#sprint-H)
- Transcript storage module (`macaw/server/transcript_store/`) with filesystem backend for persisting transcription results (#sprint-I)
- `GET /v1/speech-to-text/{id}` and `DELETE /v1/speech-to-text/{id}` endpoints for transcript retrieval and deletion (#sprint-I)
- `transcription_id` field on transcription responses when storage is enabled (#sprint-I)
- `TranscriptStoreSettings` configuration group: `MACAW_TRANSCRIPT_STORE_ENABLED`, `MACAW_TRANSCRIPT_STORE_PATH`, `MACAW_TRANSCRIPT_STORE_TTL_SECONDS` env vars (#sprint-I)
- Entity detection module (`macaw/postprocessing/entity_detection/`) with regex-based PII/PHI/PCI detector: email, phone, SSN, MRN, credit card, IP address (#sprint-G)
- `entity_detection` parameter on `POST /v1/audio/transcriptions` for entity detection in transcription output — `verbose_json` response includes `entities` list (#sprint-G)
- `DetectedEntityResponse` model and `entities` field on `VerboseTranscriptionResponse` (#sprint-G)
...
- `WebhookDelivery.deliver()` retry loop now catches `(httpx.HTTPError, OSError)` instead of bare `Exception` — programming errors bubble up immediately (#code-review-fixups)
- `async_jobs.run_job()` parameters typed properly: `coro: Awaitable[dict[str, Any]]`, `delivery: WebhookDelivery` — replaces `Any` (#code-review-fixups)
...
### Fixed
- Pronunciation store file I/O now uses explicit `encoding="utf-8"` on all `open()` calls — prevents encoding issues on non-UTF-8 systems (#code-review-fixups)

### Security
- SSML parser now uses `defusedxml.ElementTree.fromstring()` instead of stdlib `xml.etree.ElementTree.fromstring()` — prevents XML bomb (billion laughs) and XXE attacks on untrusted SSML input (#security-ssml-parser)
